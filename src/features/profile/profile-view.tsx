...
export function ProfileView() {
  ...
  const tier = tierByKey(user.tier)
  const totalPreds = Object.keys(predictions).length
  const correct = Object.values(predictions).filter((p) => p.status === 'won').length
  const accuracy = totalPreds ? Math.round((correct / totalPreds) * 100) : 0

  return (
    <div className="animate-fade-in-up pb-3">
      <section className="bg-brand px-5 pb-8 pt-7 text-center">
        <div className="mx-auto mb-3 flex h-20 w-20 items-center justify-center rounded-full bg-gold font-serif text-3xl font-bold text-dark">
          {initials(user.name)}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 px-4 pb-1 pt-2">
        <Stat value={totalPreds} label="Predictions" />
      ...
      >
        <div>
          ...
          <div className="mt-0.5 text-[13px] text-muted">{user.season_points} season points</div>
        </div>
